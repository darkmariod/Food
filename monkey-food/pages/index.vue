<template>
  <div class="min-h-screen bg-gray-50">
    <!-- Header -->
    <header class="bg-green-600 text-white p-3 sm:p-4 shadow-lg sticky top-0 z-40">
      <div class="max-w-md mx-auto flex justify-between items-center">
        <h1 class="text-lg sm:text-xl font-bold">Monkey Food</h1>
        <NuxtLink to="/admin-login" class="text-xs sm:text-sm opacity-80 hover:opacity-100 bg-white/20 px-2 py-1 rounded">
          Admin
        </NuxtLink>
      </div>
    </header>
    ...
    <div class="bg-green-50 p-2 sm:p-3">
      <p class="text-center text-green-800 text-xs sm:text-sm font-medium">
        {{ bowlsDisponibles > 0 ? `Solo ${bowlsDisponibles} bowls disponibles` : 'Pedidos cerrados hoy' }}
      </p>
      <p class="text-center text-green-700 text-xs mt-1">
        Pedidos hasta las 2PM | Entregas 5:30-7PM
      </p>
    </div>

    <!-- Beneficios -->
    <div class="bg-white border-b border-gray-100 p-3">
      <div class="max-w-md mx-auto flex flex-col gap-1 text-xs text-gray-600">
        <p><strong>30-40g proteína</strong> - ideal post-entreno</p>
        <p><strong>Ingredientes frescos</strong> - preparado cada día</p>
        <p><strong>Pide en 1 minuto</strong> - sin注册的</p>
      </div>
    </div>

    <!-- Confianza -->
    <div class="bg-gray-50 p-2 border-b border-gray-200">
      <div class="max-w-md mx-auto flex justify-center gap-4 text-xs text-gray-500">
    ...
    <!-- Carrito flotante -->
    <div v-if="menuStore.carrito.length > 0" class="fixed bottom-4 left-4 right-4 z-50 sm:left-auto sm:right-6 sm:w-auto">
      <button 
      ...
      >
        ...
        <span class="bg-white text-green-600 px-2 py-0.5 rounded-full font-bold">
    ...
    <!-- Menú -->
    <main class="max-w-md mx-auto p-3 sm:p-4 pb-24" :class="{'opacity-50 pointer-events-none': bowlsDisponibles === 0}">
      <!-- Proteína Animal -->
      <section class="mb-6">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <h3 class="font-bold text-gray-800 text-center text-sm leading-tight">{{ bowl.nombre }}</h3>
            <p class="text-xs text-gray-500 text-center mt-1 line-clamp-2">{{ bowl.descripcion }}</p>
            <p class="text-xs text-green-600 text-center mt-1 font-medium">{{ bowl.protein_grams }} proteína</p>
    ...
    <!-- Modal Carrito -->
    <div v-if="mostrarCarrito" class="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center sm:justify-center">
      <div class="bg-white w-full max-w-md mx-auto rounded-t-2xl sm:rounded-2xl p-4 max-h-[85vh] overflow-y-auto">
        ...
        <div v-if="menuStore.carrito.length === 0" class="text-center py-8 text-gray-500">
          <p class="text-4xl mb-2">🛒</p>
          <p>Tu carrito está vacío</p>
        </div>

        <div v-else class="space-y-3 mb-4">
          <div v-for="item in menuStore.carrito" :key="item.product.id" class="flex justify-between items-center border-b pb-3">
            <div class="flex-1">
              <p class="font-medium text-sm">{{ item.product.nombre }}</p>
              <p class="text-xs text-gray-500">x{{ item.cantidad }}</p>
            </div>
            <div class="flex items-center gap-3">
              <span class="font-bold">${{ item.product.price * item.cantidad }}</span>
              <button 
        ...
        <div v-if="menuStore.carrito.length > 0" class="border-t pt-4 mb-4">
          <div class="flex justify-between text-xl font-bold">
            <span>Total:</span>
            <span class="text-green-600">${{ menuStore.totalCarrito }}</span>
          </div>
        </div>
        ...
        <button 
          @click="enviarPedido"
          :disabled="!nombreCliente || !teléfonoCliente || !direcciónCliente || menuStore.carrito.length === 0 || bowlsDisponibles === 0"
...
const enviarPedido = async () => {
  ...
  const bowlsRestantes = Math.max(0, bowlsDisponibles.value - menuStore.carrito.reduce((sum, item) => sum + item.cantidad, 0))
  ...
  const itemsAgrupados = menuStore.carrito.reduce((acc, item) => {
